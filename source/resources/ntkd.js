...
AlertMeSoon.add = function(template)
{
    ...
    $('#scenarios').append(template({alert: a, colors: this.colors, types: this.types}));
...
{
    var buttons = $('.ntkd-add-scenario');
    
    buttons.on('click', function(e) {AlertMeSoon.add(AlertMeSoon.template);});
    buttons.on('mouseover', function(e) { $(e.target).animate({opacity: 1.0}, 200); });
    buttons.on('mouseout',  function(e) { $(e.target).animate({opacity: 0.5}, 200); });
});
...
Alert.prototype.initialize = function()
{
    ...
    this.dom['remove']         = document.getElementById(this.id + '-remove');
    ...
    $(this.dom['remove']).on('click', null, this, function(e) {e.data.remove();});
